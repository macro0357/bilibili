...
import com.bilibili.service.util.ImageUtil;
...
//            Set<Long> childUserIdList = childCommentList.stream().map(VideoComment::getReplyUserId).collect(Collectors.toSet());
            userIdList.addAll(replyUserIdList); //包括了一级评论和二级评论的user
//            userIdList.addAll(childUserIdList);
            List<UserInfo> userInfoList = userService.batchGetUserInfoByUserIds(userIdList);
            Map<Long, UserInfo> userInfoMap = userInfoList.stream().collect(Collectors.toMap(UserInfo :: getUserId, userInfo -> userInfo));
            list.forEach(comment -> { //双重循环匹配二级评论
                Long id = comment.getId();
                List<VideoComment> childList = new ArrayList<>();
                childCommentList.forEach(child -> {
                    if(id.equals(child.getRootId())){ // 通过comment的主键id判断
                        child.setUserInfo(userInfoMap.get(child.getUserId())); //设置自己的用户信息
                        child.setReplyUserInfo(userInfoMap.get(child.getReplyUserId())); //设置回复的用户的信息（因为可能不是回复楼主）
                        childList.add(child);
                    }
                });
    ...
    public Map<String, Object> getVideoDetails(Long videoId) {
        Video video =  videoDao.getVideoDetails(videoId); //getVideoById()相同功能
    ...
    public List<VideoBinaryPicture> convertVideoToImage(Long videoId, String fileMd5) throws Exception{
        com.bilibili.domain.File file = fileService.getFileByMd5(fileMd5);
