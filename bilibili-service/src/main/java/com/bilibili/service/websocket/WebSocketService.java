...
public class WebSocketService {
    ...
    public void openConnection(Session session, @PathParam("token") String token) {
        ...
        logger.info("用户连接成功：" + sessionId + "，当前在线人数人：" + ONLINE_COUNT.get());
        try {
            this.sendMessage("0"); //连接成功
        } catch (Exception e) {
            logger.error("连接异常！");
        }
    }
        ...
        if (!StringUtils.isNullOrEmpty(message)) {
            try {
                ...
                for (Map.Entry<String, WebSocketService> entry : WEBSOCKET_MAP.entrySet()) {
                    ...
                    Message msg = new Message(UserMomentsConstant.TOPIC_DANMUS, jsonObject.toString().getBytes(StandardCharsets.UTF_8));
                ...
                if (this.userId != null) {
                    ...
                    danmuService.asyncAddDanmu(danmu); // 异步保存到redis
                    //保存弹幕到redis
